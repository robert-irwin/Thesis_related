...
# define VSET (float) 110.0 // desired voltage at the load
# define kp (float) .9 // proportional gain
# define ki (float) 0.09 // integral gain
# define MATROWS N
# define MATCOLS 2
  ...
  while(1)
      ...
      for (int i = 0; i < N; i++)
	{
      ...
      *RMS_res0 = *RMS_res0 * 65.5367;
      ...
      //printf("%f   %f\n", Verr_new, Verr_old);
      temp =  ((kp * (Verr_new - Verr_old)) + ((ki * (Verr_new + Verr_old)/2) * Ts)); // PI control
      printf("Error Calculation: %f\n", temp);
      d_float = old_d + temp;
      Verr_old = Verr_new; // store the most current error signal
      old_d = d_float; // store the most recent duty cycle
      new_d = (int) d_float;
