...
char myMACADDR[HWADD_LEN] = {0x90, 0xA2, 0xDA, 0x10, 0xA3, 0xB2};
...
char sIP[IP_LEN] = {0xC0, 0xA8, 0x00, 0x01};
char recvIP[IP_LEN] = {0x00, 0x00, 0x00, 0x00};
w5500packet	 PAC;
w5500packet * pac = &PAC;
...
int main (void)
{
	...
	*dataBuf = 0x1A;
	...
	OCR2B = 132; // initial 50% duty cycle on pin OC2A
	...
	while(1)
	{
		...
		memcpy(recvIP, dataBuf, IP_LEN); // get the sIP from the header
		if (strncmp(sIP, recvIP, IP_LEN) == 0)
			OCR2B = *(dataBuf+8);
